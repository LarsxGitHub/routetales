...
class TestHasSiblingInPyt(unittest.TestCase):
  def test_false(self):
    pyt = pyt_from_pfxs_v4(["192.0.2.0/24", "198.51.100.0/25"])
    pfx = ipaddress.IPv4Network("192.0.2.0/24")
    self.assertFalse(has_sibling_in_pyt(pyt, pfx))

  def test_true_upwards(self):
    pyt = pyt_from_pfxs_v4(["192.0.2.0/24", "192.0.3.0/24"])
    pfx = ipaddress.IPv4Network("192.0.2.0/24")
    self.assertTrue(has_sibling_in_pyt(pyt, pfx))

  def test_true_downwards(self):
    pyt = pyt_from_pfxs_v4(["192.0.2.0/24", "192.0.3.0/24"])
    pfx = ipaddress.IPv4Network("192.0.3.0/24")
    self.assertTrue(has_sibling_in_pyt(pyt, pfx))


class TestHasDirectLessSpecificInPyt(unittest.TestCase):
  def test_true(self):
    pyt = pyt_from_pfxs_v4(["192.0.2.0/24", "192.0.2.0/23"])
    pfx = ipaddress.IPv4Network("192.0.2.0/24")
    self.assertTrue(has_direct_less_specific_in_pyt(pyt, pfx))

  def test_false_indirect(self):
    pyt = pyt_from_pfxs_v4(["192.0.2.0/24", "192.0.0.0/22"])
    pfx = ipaddress.IPv4Network("192.0.2.0/24")
    self.assertFalse(has_direct_less_specific_in_pyt(pyt, pfx))

  def test_false_nonexist(self):
    pyt = pyt_from_pfxs_v4(["192.0.2.0/24"])
    pfx = ipaddress.IPv4Network("192.0.2.0/24")
    self.assertFalse(has_direct_less_specific_in_pyt(pyt, pfx))
